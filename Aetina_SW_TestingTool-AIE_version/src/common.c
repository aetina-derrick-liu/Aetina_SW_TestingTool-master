...
void uploadReport(TestReport *report){
	char *ftp_ip = NULL;
	char *ftp_account = NULL;
	char *ftp_password = NULL;
	ftp_ip = checkConfigInfo("ftpip", ftp_ip);
	ftp_account = checkConfigInfo("ftpaccount", ftp_account);
	ftp_password = checkConfigInfo("ftppassword", ftp_password);
...
void configureAutoRunItems(TestCore *core){
	...
	if (strstr(board, "AIE-CN11")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CN12")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CN13")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CN14")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	if (strstr(board, "AIE-CT41")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CT42")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CT43")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}
	else if (strstr(board, "AIE-CT44")){
		if (checkConfigItem(items_name[CANbus], &szValue) == OK && szValue < 0){
		core->pTestItems = addItem(core->pTestItems, CANbus);
		szValue = 0;
	}
	}			
	else if (strstr(board, "AIE-CO11")){
	}
	else if (strstr(board, "AIE-CO12")){
	}
	else if (strstr(board, "AIE-CO13")){
	}
	else if (strstr(board, "AIE-CO14")){
	}	
	#ifdef _DEBUG_
...
void setCfgNRst_TX2_NX(const ProductPortFolio folio, Items *items, TestConfigs *configs, TestResults *results){
	...
	memset(usb_3_micro_dev, 0, sizeof(usb_3_micro_dev));
 	...
 	char usb_3_type_a_down_usb3_dev[] = "usb2/2-2/2-2.1";
  	char usb_3_type_a_donw_usb2_dev[] = "usb1/1-2/1-2.1";
	char usb_3_type_a_up_usb3_dev[] = "usb2/2-2/2-2.2";
	char usb_3_type_a_up_usb2_dev[] = "usb1/1-2/1-2.2";
	//SKU3 210610_mental
	//2nd
 	char usb_2nd_type_a_down_usb3_dev[] = "usb4/4-1/4-1.2";
  	char usb_2nd_type_a_down_usb2_dev[] = "usb3/3-1/3-1.1";
	char usb_2nd_type_a_up_usb3_dev[] = "usb4/4-2/4-2.2";
	char usb_2nd_type_a_up_usb2_dev[] = "usb3/3-2/3-2.1";
	//3rd
 	char usb_3rd_type_a_down_usb3_dev[] = "usb6/6-1/6-1.2";
  	char usb_3rd_type_a_down_usb2_dev[] = "usb5/5-1/5-1.1";
	char usb_3rd_type_a_up_usb3_dev[] = "usb6/6-2/6-2.2";
	char usb_3rd_type_a_up_usb2_dev[] = "usb5/5-2/5-2.1";
	//4th
 	char usb_4th_type_a_down_usb3_dev[] = "usb8/8-1/8-1.2";
  	char usb_4th_type_a_down_usb2_dev[] = "usb7/7-1/7-1.1";
	char usb_4th_type_a_up_usb3_dev[] = "usb8/8-2/8-2.2";
	char usb_4th_type_a_up_usb2_dev[] = "usb7/7-2/7-2.1";


	if (folio == AN110){
		int an110Gpio[] = {425, 30, 338, 269, 392};
		for(int i=0;i<5;i++)
		{
			gpio[i]=an110Gpio[i];
		}
	...
	while(pItem != NULL){
		...
		case LAN:
		{
			int lan_count_value = 1;
			if (folio == AIE_CT42){
				lan_count_value = 3;
			}
			else if (folio == AIE_CT41){
				lan_count_value = 2;
			}			
			else{
			...
			configs->mLAN_Count = lan_count_value;
			configs->pCfg_LAN = realloc(configs->pCfg_LAN, lan_count_value * sizeof(LAN_CFG));
			results->pResult_LAN = realloc(results->pResult_LAN, lan_count_value * sizeof(LAN_RESULT));
			memset(results->pResult_LAN, 0, lan_count_value * sizeof(LAN_RESULT));

			for (int i=0;i<lan_count_value;i++){
		...
		case USB_Type_A:
			//test usb device
			configs->pCfg_USB->nInterfaceNum = 4;
			if (folio == AIE_CT43){
				configs->pCfg_USB->nInterfaceNum = 16;
			}
			//1st
			//UP_U3
			...
			//2nd
			//UP_U3
			configs->pCfg_USB->pUSB_2nd_TYPE_A_UP_USB3DEV = malloc(strlen(usb_2nd_type_a_up_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_2nd_TYPE_A_UP_USB3DEV, usb_2nd_type_a_up_usb3_dev);
			//UP_U2
			configs->pCfg_USB->pUSB_2nd_TYPE_A_UP_USB2DEV = malloc(strlen(usb_2nd_type_a_up_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_2nd_TYPE_A_UP_USB2DEV, usb_2nd_type_a_up_usb2_dev);
			//DOWN_U3
			configs->pCfg_USB->pUSB_2nd_TYPE_A_DOWN_USB3DEV = malloc(strlen(usb_2nd_type_a_down_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_2nd_TYPE_A_DOWN_USB3DEV, usb_2nd_type_a_down_usb3_dev);
			//DOWN_U2
			configs->pCfg_USB->pUSB_2nd_TYPE_A_DOWN_USB2DEV = malloc(strlen(usb_2nd_type_a_down_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_2nd_TYPE_A_DOWN_USB2DEV, usb_2nd_type_a_down_usb2_dev);
			//3rd
			//UP_U3
			configs->pCfg_USB->pUSB_3rd_TYPE_A_UP_USB3DEV = malloc(strlen(usb_3rd_type_a_up_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3rd_TYPE_A_UP_USB3DEV, usb_3rd_type_a_up_usb3_dev);
			//UP_U2
			configs->pCfg_USB->pUSB_3rd_TYPE_A_UP_USB2DEV = malloc(strlen(usb_3rd_type_a_up_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3rd_TYPE_A_UP_USB2DEV, usb_3rd_type_a_up_usb2_dev);
			//DOWN_U3
			configs->pCfg_USB->pUSB_3rd_TYPE_A_DOWN_USB3DEV = malloc(strlen(usb_3rd_type_a_down_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3rd_TYPE_A_DOWN_USB3DEV, usb_3rd_type_a_down_usb3_dev);
			//DOWN_U2
			configs->pCfg_USB->pUSB_3rd_TYPE_A_DOWN_USB2DEV = malloc(strlen(usb_3rd_type_a_down_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3rd_TYPE_A_DOWN_USB2DEV, usb_3rd_type_a_down_usb2_dev);			
			//4th
			//UP_U3
			configs->pCfg_USB->pUSB_4th_TYPE_A_UP_USB3DEV = malloc(strlen(usb_4th_type_a_up_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_4th_TYPE_A_UP_USB3DEV, usb_4th_type_a_up_usb3_dev);
			//UP_U2
			configs->pCfg_USB->pUSB_4th_TYPE_A_UP_USB2DEV = malloc(strlen(usb_4th_type_a_up_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_4th_TYPE_A_UP_USB2DEV, usb_4th_type_a_up_usb2_dev);
			//DOWN_U3
			configs->pCfg_USB->pUSB_4th_TYPE_A_DOWN_USB3DEV = malloc(strlen(usb_4th_type_a_down_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_4th_TYPE_A_DOWN_USB3DEV, usb_4th_type_a_down_usb3_dev);
			//DOWN_U2
			configs->pCfg_USB->pUSB_4th_TYPE_A_DOWN_USB2DEV = malloc(strlen(usb_4th_type_a_down_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_4th_TYPE_A_DOWN_USB2DEV, usb_4th_type_a_down_usb2_dev);
			break;
		case EEPROM:
			configs->pCfg_EEPROM->nI2C_ID = 7;
			configs->pCfg_EEPROM->nAddress = 0x50;
		...
		case Camera:
			...
			break;
		case Temperature:
			configs->pCfg_Temperature->nTemperature_i2c_ID = 1;
...
void setCfgNRst_Xavier_NX(const ProductPortFolio folio, Items *items, TestConfigs *configs, TestResults *results){
	...
	int gpio[] = {421, 424, 268, 393, 491};

 	char usb_3_type_a_down_usb3_dev[] = "usb2/2-3/2-3.1";
  	char usb_3_type_a_down_usb2_dev[] = "usb1/1-2/1-2.1";
	char usb_3_type_a_up_usb3_dev[] = "usb2/2-3/2-3.2";
	char usb_3_type_a_up_usb2_dev[] = "usb1/1-2/1-2.2";
	//SKU3 210609_mental
	//2nd
 	char usb_2nd_type_a_down_usb3_dev[] = "usb4/4-1";
  	char usb_2nd_type_a_down_usb2_dev[] = "usb3/3-1";
	char usb_2nd_type_a_up_usb3_dev[] = "usb4/4-2";
	char usb_2nd_type_a_up_usb2_dev[] = "usb3/3-2";
	//3rd
 	char usb_3rd_type_a_down_usb3_dev[] = "usb6/6-1";
  	char usb_3rd_type_a_down_usb2_dev[] = "usb5/5-1";
	char usb_3rd_type_a_up_usb3_dev[] = "usb6/6-2";
	char usb_3rd_type_a_up_usb2_dev[] = "usb5/5-2";
	//4th
 	char usb_4th_type_a_down_usb3_dev[] = "usb8/8-1";
  	char usb_4th_type_a_down_usb2_dev[] = "usb7/7-1";
	char usb_4th_type_a_up_usb3_dev[] = "usb8/8-2";
	char usb_4th_type_a_up_usb2_dev[] = "usb7/7-2";
	//
	...
	while(pItem != NULL){
		...
		case USB_Type_A:
			...
			configs->pCfg_USB->pUSB_3_0_TYPE_A_DOWN_USB3DEV = malloc(strlen(usb_3_type_a_down_usb3_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3_0_TYPE_A_DOWN_USB3DEV, usb_3_type_a_down_usb3_dev);
			configs->pCfg_USB->pUSB_3_0_TYPE_A_DOWN_USB2DEV = malloc(strlen(usb_3_type_a_down_usb2_dev)*sizeof(char));
			strcpy(configs->pCfg_USB->pUSB_3_0_TYPE_A_DOWN_USB2DEV, usb_3_type_a_down_usb2_dev);
			break;
		case m2_eKey:
		...
		case Camera:
			break;
		case Temperature:
			configs->pCfg_Temperature->nTemperature_i2c_ID = 8;
			configs->pCfg_Temperature->nTemperature_i2c_Address = 0x48;
			break;
		default:
			break;
...
void setCfgNRst_Nano(const ProductPortFolio folio, Items *items, TestConfigs *configs, TestResults *results){
	...
	//2nd
 	char usb_2nd_type_a_down_usb3_dev[] = "usb4/4-1";
  	char usb_2nd_type_a_down_usb2_dev[] = "usb4/4-2";
	char usb_2nd_type_a_up_usb3_dev[] = "usb3/3-1";
	char usb_2nd_type_a_up_usb2_dev[] = "usb3/3-2";
	//3rd
 	char usb_3rd_type_a_down_usb3_dev[] = "usb5/5-1";
  	char usb_3rd_type_a_down_usb2_dev[] = "usb5/5-2";
	char usb_3rd_type_a_up_usb3_dev[] = "usb6/6-2";
	char usb_3rd_type_a_up_usb2_dev[] = "usb6/6-1";
	//4th
 	char usb_4th_type_a_down_usb3_dev[] = "usb8/8-1";
  	char usb_4th_type_a_down_usb2_dev[] = "usb8/8-2";
	char usb_4th_type_a_up_usb3_dev[] = "usb7/7-1";
	char usb_4th_type_a_up_usb2_dev[] = "usb7/7-2";
	//
	char eeprom_data[] = "Aetina Corporation";
...
void configureTestCore(TestCore *core, TestMode mode){
	...
	if (strstr(module, "Xavier_NX")){
		if (strstr(dts, "CN11")){
			setCfgNRst_Xavier_NX(AIE_CN11, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CN12")){
			setCfgNRst_Xavier_NX(AIE_CN12, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CN13")){
			setCfgNRst_Xavier_NX(AIE_CN13, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CN14")){
			setCfgNRst_Xavier_NX(AIE_CN14, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
	}
	else if (strstr(module, "TX2-NX")){
		if (strstr(dts, "CT41")){
			setCfgNRst_TX2_NX(AIE_CT41, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CT42")){
			setCfgNRst_TX2_NX(AIE_CT42, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CT43")){
			setCfgNRst_TX2_NX(AIE_CT43, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CT44")){
			setCfgNRst_TX2_NX(AIE_CT44, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
	}	
	else if (strstr(module, "Nano")){
		if (strstr(dts, "CO11")){
			setCfgNRst_Nano(AIE_CO11, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CO12")){
			setCfgNRst_Nano(AIE_CO12, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CO13")){
			setCfgNRst_Nano(AIE_CO13, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
		else if (strstr(dts, "CO14")){
			setCfgNRst_Nano(AIE_CO14, core->pTestItems, core->pTestConfigs, core->pTestResults);
		}
	}
}
